...
1. In the  **Jenkins-Ansible** server and create a new directory called **ansible-config-artifact** – where we will store all artifacts after each build.
...
restarting Jenkins.
4. Create a new **Freestyle project** ([see Jenkins_Config_Installatiion](https://github.com/Gshare-dev/Continuous-Integration-Pipeline-With-Jenkins/blob/main/Jenkins_Config_Installatiion.md)) and name it 
**save_artifacts**.

5. This project will be triggered upon the completion of the existing ansible project. Configure it accordingly:
...
In Ansible_Mgt_Congif project I wrote all tasks in a single playbook **common.yml**, a simple set of instructions for only 2 types of OS, but imagine there are many more tasks 
...
3. Create a new folder in root of the repository and name it **static-assignments**. The static-assignments folder is where all other children playbooks will be stored. This is merely for easy organization of your work. It is not an Ansible specific concept.
